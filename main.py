...
@bot.on_message(pyrogram.filters.command("start") & pyrogram.filters.incoming)
async def start(client: pyrogram.client.Client, message: pyrogram.types.Message):

    if users.is_exists({"id": message.from_user.id}):
    ...
    await message.reply(
        text=f"Hello {name}, what is your age?"
    )
    while True:
    ...
    await message.reply(
        ...
        reply_markup=pyrogram.types.ReplyKeyboardMarkup(keyboard=[
            [
    ...
    contact_response = await chat.listen(filters=(
        pyrogram.filters.contact
    )
